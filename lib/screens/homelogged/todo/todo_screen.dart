  ...
  Widget build(BuildContext context) {
    ...
    final inProgressRequest = maintenanceRequestProvider.inProgressRequest;
     ...
     List<String> requestImages =
        inProgressRequest?.images is List<String> ? List<String>.from(inProgressRequest!.images) : [];
    String requestImage = requestImages.isNotEmpty ? requestImages.first : '';
    print("request Image: $requestImage");
    String address = "${inProgressRequest?.room?.property?.address.line1 ?? "Unknown line1"}, ${inProgressRequest?.room?.property?.address.line2 ?? "Unknown line2"}, ${inProgressRequest?.room?.property?.address.province ?? "Unknown provice"}, ${inProgressRequest?.room?.property?.address.postalCode ?? "Unknown postalCode"} ";
    return Scaffold(
      // appBar: AppBar(
          ...
          child: SingleChildScrollView(
            child: Column(
              children: [
                ...
                if (inProgressRequest != null) Card(
                        // elevation: 5.0, // Card elevation for better visual appeal
                        shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(12.0), // Rounded corners
                        ),
                        margin: EdgeInsets.symmetric(vertical: 10.0), // Margin around card
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  Icon(Icons.assignment, color: Colors.blueAccent), // Icon for task
                                  SizedBox(width: 10),
                                  Text(
                              ...
                              ElevatedButton(
                                onPressed: () {
                                  ...
                                  Provider.of<MaintenanceRequestProvider>(context, listen: false).completeMaintenanceRequest(context, inProgressRequest.id);
